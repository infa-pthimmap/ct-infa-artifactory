...
var (
	...
	infaMetricsPath = kingpin.Flag("web.infa-telemetry-path", "Path under which to expose metrics.").Envar("INFA_WEB_TELEMETRY_PATH").Default("/infametrics").String()
...
type DockerCredentials struct {
	Username string `required:"false" envconfig:"DOCKER_USERNAME"`
	Password string `required:"false" envconfig:"DOCKER_PASSWORD"`
}
...
type Config struct {
	...
	InfaListenAddress string
	MetricsPath       string
	InfaMetricsPath   string
...
func NewConfig() (*Config, error) {
	...
	var dockerCredentials DockerCredentials
	docker_err := envconfig.Process("", &dockerCredentials)
	if docker_err != nil {
		return nil, docker_err
	}
	if dockerCredentials.Username != "" && dockerCredentials.Password != "" {
		fmt.Println("Docker credentials are set")
	} else {
		return nil, fmt.Errorf("`DOCKER_USERNAME` and `DOCKER_PASSWORD`  environment variable hast to be set")
	}
	...
	return &Config{
		...
		InfaMetricsPath:   *infaMetricsPath,
