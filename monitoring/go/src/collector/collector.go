...
var (
	...
	/**dockerMetrics = metrics{
		"docker_status": newMetric("docker_stats", "docker", "docker stats.", defaultLabelNames),
		"helm_status":   newMetric("helm_status", "docker", "helm stats", append([]string{"status"}, defaultLabelNames...)),
	}*/
)

func init() {
	Registry.MustRegister(version.NewCollector("artifactory_exporter"))
